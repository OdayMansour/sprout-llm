...
export function SuggestionPopup({ anchorX, anchorY, onAskHere, onFork, onDismiss }: Props) {
      ...
      {expanded ? (
        <div className="bg-gray-800 border border-gray-600 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b border-gray-700">
            <SparkleIcon />
            <span className="flex-1 text-xs text-teal-400 font-medium">AI suggestion</span>
            <button
          ...
          <div className="p-2 flex flex-col gap-2">
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="px-3 py-1.5 text-xs font-medium bg-gray-700 border border-gray-600 text-gray-200 rounded-lg hover:bg-gray-600 transition-colors"
              >
                Ask here
              </button>
              <button
                onClick={() => onFork(question.trim() || DEFAULT_QUESTION)}
                className="px-3 py-1.5 text-xs font-medium bg-teal-700 text-white rounded-lg hover:bg-teal-600 transition-colors"
              >
                New thread →
              </button>
            </div>
        ...
        <div className="inline-flex items-center bg-gray-800 border border-gray-600 rounded-full shadow-xl overflow-hidden">
          <button
            ...
            className="flex items-center gap-1.5 pl-3 pr-2.5 py-2.5 text-xs font-medium text-teal-400 hover:bg-gray-700 transition-colors"
          >
            <SparkleIcon />
            Ask here
          </button>
          ...
          <button
            ...
            className="px-2.5 py-2.5 text-xs text-gray-400 hover:text-gray-200 hover:bg-gray-700 transition-colors"
